...
\label{sm}
Due to the observation strategy in \scanmode, each source samples a cord across the entire detector. 
%If a chunk of photons are stacked on the detector, the number of photons on the detector can be visualized as Fig.~\ref{photons}, which contains not only signatures from stars passing across the detector, but also the features of sensitivity-map.
If every star is assumed to be constant in brightness, the counts of photons in pixel $(x,y)$ of star $i$, $C_{i,x,y}$ can be modelled as:
\begin{eqnarray}
