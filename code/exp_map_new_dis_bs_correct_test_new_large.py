...
def exp_dead_new(file_num, name_file, imsz, wcs, hrflat, foc_list, asp_solution, dead, cut, step, out_path, return_dict):
    print imsz
    count = np.zeros(imsz)
    ...
    print start.shape
    ox = np.repeat(np.arange(l)+start,length+400)
    oy = np.tile(np.arange(length+400)+foc_list[0,0]-half_len-200,l)
    ...
    for i in range(asp_solution.shape[0]):
        ...
        if (foc[1]+half_len)>=(start+l):
            ...
            ox = np.repeat(np.arange(l)+start,length+400)
            oy = np.tile(np.arange(length+400)+foc[0]-half_len-200,l)
            omask = (ox>=0) & (ox<imsz[0]) & (oy>=0) & (oy<imsz[1])
            ox = ox[omask]
            oy = oy[omask]
            gl,gb = wcs.all_pix2world(oy,ox,0)
...
#process
    if True:
        ...
        flat = np.load('/scratch/dw1519/galex/data/star_photon/super_iterate1/flat.npy')
        resample=3 #4
        ...
        for file_num in range(0, num_t):
            p = Process(target=exp_dead_new, args=(file_num, name_file, imsz, wcs, hrflat, foc_list[file_num], asp_solutions[file_num], deads[file_num], cuts[file_num], step, out_path, return_dict))
            p.start()
            p_list.append(p)
