...
def dis_correct(data, dis_map, asp, slope, inter):
    ...
    if slope is not None:
        data_new[:,5] -= (slope*data_new[:,2]+inter)*36000*800*0.001666/2400.

    ra, dec = gn.gnomrev_simple(data_new[:,4], data_new[:,5], asp[ix,1], asp[ix,2], -asp[ix,3],1/36000.,0.)
    return np.concatenate([np.array([ra,dec]).T, data[ya_mask&q_mask,0:6], asp[ix,1:4]], axis=1)
...
def load_data(name, dis_l, dis_h, ya, c_file, asprta, resolution):
    ...
    for f in photon_list:
        ...
        catalog_id = np.zeros((gal.shape[0], 4))-1
        #catalog_id[idxcatalog,0] = 
        if len(idxc)>0:       
            catalog_id[idxcatalog] = np.column_stack([idx[mask][idxc], tycho2['RAJ2000'][idx[mask][idxc]],\
                                    tycho2['DEJ2000'][idx[mask][idxc]], df['nuv'][mask][idxc]])
        else:
            print 'no star'
        #catalog_id = np.array([catalog_id]).T
        data = np.concatenate([data[:,2:8],catalog_id], axis=1)
...
if __name__ == '__main__':
    if True:
        ...
        np.save('../data/'+name+'-cal-sec/photon_match.npy', data)
