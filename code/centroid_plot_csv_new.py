...
def generate_new_offsets_new(name, asprta, suffix, tmp_dir):
    print name
    hdulist = pyfits.open('../AIS_GAL_SCAN/asprta/%s-asprta.fits'%name)
    initial = 0
    final = hdulist[1].data['T'].shape[0]-1
    ...
    print centroids.shape
    print time.shape
    output = '../plots/%s/cata/offsets_%s.pdf'%(name, suffix)
    dir = os.path.dirname(output)
    if not os.path.exists(dir):
      os.makedirs(dir)
    plt.plot(centroids[:,0], '.b')
    plt.savefig('../plots/%s/cata/offsets_%s.pdf'%(name, suffix), dpi=190)
    plt.clf()

    np.save(tmp_dir+'/offsets%d_%s.npy'%(initial, suffix), centroids)
    np.save(tmp_dir+'/time%d_%s.npy'%(initial, suffix), time)
    ...
    tmp_files = glob.glob(tmp_dir+"/centroids_rot*")
    for tmp_file in tmp_files:
        os.remove(tmp_file)

    tmp_files = glob.glob(tmp_dir+"/time_rot*")
    for tmp_file in tmp_files:
        os.remove(tmp_file)
