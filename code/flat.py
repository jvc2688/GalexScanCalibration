    ...
    for scan in scans:
        ...
        for name in name_list:
            ...
            for f in csvs:
                ...
                #idxc, idxcatalog, d2d_p, d3d_p = sky_data.search_around_sky(c, 0.0022*u.deg)
                idxc, idxcatalog, d2d_p, d3d_p = sky_data.search_around_sky(c, 0.0033*u.deg)
                if len(idxc)>0:
                    photon_list.append(np.column_stack([idxc, data[idxcatalog,0], data[idxcatalog,3:]]))
                else:
                    print 'no star'
            photon_list = np.concatenate(photon_list, axis=0)
            np.save('/scratch/dw1519/galex/data/star_photon/'+outdir+'/'+name+'_photon.npy', photon_list)
            np.save('/scratch/dw1519/galex/data/star_photon/'+outdir+'/'+name+'_star.npy', np.array([df['gl'], df['gb'], df['nuv']]).T)
