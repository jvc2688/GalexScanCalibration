...
#process
	if True:
		...
		for name in name_list:
			try:
				asp_solution_tmp = np.load('../data/photon_list/{0}{1}-new-sp_asp_new.npy'.format(name, suffix))
			except IOError:
				...
				dec = co_data['dec']
				roll = co_data['roll']
				t_new = np.arange((T.shape[0]-1)*100)*0.005+T[0]
				...
				np.save('../data/photon_list/{0}{1}-new-sp_asp_new.npy'.format(name, suffix), asp_solution_tmp)
		...
		hdulist.writeto('../fits/scan_map/count_map_{0}{1}_exp_fix.fits'.format(name_file, suffix), clobber=False)
