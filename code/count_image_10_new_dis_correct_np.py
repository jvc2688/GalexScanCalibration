...
#process interpolate fast
  if True:
    ...
    for name in name_list:
      data = glob.glob("{0}/{1}.npy".format(data_path, name))
      csv_lists[name] = data
      num = len(data)
      ...
      guide_asp = np.load('{0}/{1}_asp.npy'.format(guide_path, name))
    ...
    for csv_info in csv_info_list:
      ...
      for chunk in chunks:
        ...
        p = Process(target=run_one_inter_fast, args=(pid, name, imsz, wcs, chunk, out_path,
                                                 asp, scst_time, hv, dis_map, slope, inter, cut, return_dict))
        p.start()
        p_list.append(p)
        pid += 1
    print(pid)
    ...
    for i in range(0, pid):
      count += return_dict[i][1]#np.load('{0}/{1}_gal_sec_count_tmp{2}.npy'.format(out_path, name_file, i))
      tranges += return_dict[i][0]
