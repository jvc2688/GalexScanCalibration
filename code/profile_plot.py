...
if __name__ == '__main__':
	...
	if False:
		...
		'''
		out = '/scratch/dw1519/galex/co/co3281_2-10'
		...
		for i in range(100, 400, 400):
			...
			data_list1=[]
			for j in range(4):
				data = np.load('{0}/{1}.npy'.format(out,i+j*100))/3600.
				data1 = np.load('{0}/cut/{1}.npy'.format(out,i+j*100))/3600.
				data_list.append(data)
				data_list1.append(data1)
				#info = np.load('/home/dw1519/dw1519/galex/plots/co32-10/info_{0}.npy'.format(i))
				#mask = info[:,1]>=2]
			...
			plt.savefig('{0}/compare/{1}.png'.format(out,i))
			plt.clf()
			...
			plt.savefig('{0}/compare/cut_{1}.png'.format(out,i))
			plt.clf()
	...
	if True:
		...
		for name in name_list:
			...
			plt.plot(np.arange(s.T.shape[0], dtype=float)*qhists.shape[0]/s.shape[0], nine)
			plt.plot(np.arange(s.T.shape[0], dtype=float)*qhists.shape[0]/s.shape[0], fif)
			plt.plot(np.arange(s.T.shape[0], dtype=float)*qhists.shape[0]/s.shape[0], ten)
			...
			plt.savefig('../co/qhist/'+scan+'_new.png', dpi=190)
			plt.clf()
