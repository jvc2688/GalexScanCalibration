...
if __name__ == '__main__':
    ...
    if True:
        ...
        for name in name_list:
            ...
            print name
            if i==0:
                count = pyfits.open('../fits/scan_map/11-14-2017/cut/count_map_%scut_count.fits'%name)
                exp = pyfits.open('../fits/scan_map/11-14-2017/cut/count_map_%scut_exp.fits'%name)
                c = count[0].data
                e = exp[0].data
                c[np.isnan(c)] = 0
                c[c>5000] = 0
                ...
                ch = pyfits.open('../fits/scan_map/11-14-2017/cut/count_map_%scut_count.fits'%name)
                eh = pyfits.open('../fits/scan_map/11-14-2017/cut/count_map_%scut_exp.fits'%name)
                c = ch[0].data
                e = eh[0].data
                c[np.isnan(c)] = 0
                c[c>5000] = 0
        ...
        hdu.header = count[0].header
        hdulist = pyfits.HDUList([hdu])
        hdulist.writeto('../fits/scan_map/11-14-2017/count_map_%scut_count.fits'%sky, clobber=False)

        hdu = pyfits.PrimaryHDU(em.astype('float32'))
        hdu.header = exp[0].header
        hdulist = pyfits.HDUList([hdu])
        hdulist.writeto('../fits/scan_map/11-14-2017/count_map_%scut_exp.fits'%sky, clobber=False)
        '''
        count = pyfits.open('../fits/scan_map/11-14-2017/count_map_%scut_count.fits'%sky)
        cm = count[0].data

        exp = pyfits.open('../fits/scan_map/11-14-2017/count_map_%scut_exp.fits'%sky)
        em = exp[0].data

        em[np.where(em<0.1)]=1.
        print np.min(em)
        print np.max(em)
        ...
        hdu.header = count[0].header
        hdulist = pyfits.HDUList([hdu])
        hdulist.writeto('../fits/scan_map/11-14-2017/count_map_%scut_in.fits'%sky, clobber=False)
