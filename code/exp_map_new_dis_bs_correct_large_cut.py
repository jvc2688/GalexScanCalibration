...
def exp_dead_new(file_num, name_file, imsz, wcs, hrflat, asp_solution, dead, cut, step, out_path, return_dict):
    count = np.zeros(imsz)
    ...
    for i in range(asp_solution.shape[0]):
        ...
        if i%2000==0:
            with open('%s/%s_gal_sec_exp_tmp%d.dat'%(out_path, name_file, file_num),'w') as f:
                f.write('%d'%i)
            print i
...
#process
    if True:
        ...
        flat = np.load('/scratch/dw1519/galex/data/star_photon/cut_iterate6/flat6.npy')
        resample=3 #4
        ...
        print hrflat.shape
        npix = 800*resample
        x = np.repeat(np.arange(npix),npix)
        y = np.tile(np.arange(npix),npix)
        l = np.zeros((npix,npix))
        l[x,y] = np.sqrt((x-npix/2.)**2+(y-npix/2.)**2)
        cut_mask = l>=(((0.5*36000/36000.)/(1.25/2.)*(1.25/(800* 0.001666))+1.)/2.*npix-npix/2.)
        hrflat[cut_mask] = 0
        ...
        for file_num in range(0, num_t):
            p = Process(target=exp_dead_new, args=(file_num, name_file, imsz, wcs, hrflat, asp_solutions[file_num], deads[file_num], cuts[file_num], step, out_path, return_dict))
            p.start()
            p_list.append(p)
        ...
        hdulist.writeto('{0}/count_map_{1}cut_exp.fits'.format(out_path, name_file), clobber=False)


        for i in range(num_t):
            os.remove('{0}/{1}_gal_sec_exp_tmp{2}.npy'.format(out_path, name_file, i))
            os.remove('{0}/{1}_gal_sec_exp_tmp{2}.dat'.format(out_path, name_file, i))
