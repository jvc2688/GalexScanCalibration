...
def run_one_inter_fast(pid, file_name, imsz, wcs, csv_list, out_path, asp, scst_time, hv, dis_map, ya_corr, q_corr, cut, return_dict):
  ...
  for csv_file in csv_list:
    ...
    scst_ix = np.digitize(data[:,0]/1000., scst_time)-1
...
#process interpolate fast
  if True:
    ...
    for csv_info in csv_info_list:
      ...
      scst_file.close()
      scst_time, hv = inter_hv(scst_time, hv)
      ...
      roll = co_data['roll']
      N = 1./0.005
      t_new = np.arange((T[-1]-T[0])*N+1)*0.005+T[0]#np.arange((T.shape[0]-1)*n)*0.005+T[0]
      scst_ix = np.digitize(t_new, scst_time)-1
      ix_mask = (scst_ix<scst_time.shape[0])&(scst_ix>=0)
      scst_ix = scst_ix[ix_mask]
      t_new = t_new[ix_mask]
      t_new = t_new[hv[scst_ix]>0]
      #asp_sky = SkyCoord(ra,dec, unit='deg', frame=FK5, equinox='J2000.0')
      #asp_gal = asp_sky.transform_to(Galactic)

      #spl_gl = splrep(T, asp_gal.l.deg)
      #spl_gb = splrep(T, asp_gal.b.deg)
      spl_ra = splrep(T, ra)
      spl_dec = splrep(T, dec)
      spl_roll = splrep(T, roll)
      ra_new = splev(t_new, spl_ra)
      dec_new = splev(t_new, spl_dec)
      #gl_new = splev(t_new, spl_gl)
      #gb_new = splev(t_new, spl_gb)
      #asp_gal_new = SkyCoord(gl_new,gb_new, unit='deg', frame=FK5, equinox='J2000.0')
      #roll_new = splev(t_new, spl_roll)
      roll_new = inter_angle(t_new, T, roll)

      asp = np.array([t_new, ra_new, dec_new, roll_new]).T
      plt.plot(t_new, ra_new, '.k')
      plt.savefig('/scratch/dw1519/galex/plots/inter/{0}.pdf'.format(name))
      plt.clf()
    ...
    for i in range(0, pid):
      count += np.load('{0}/{1}_gal_sec_count_tmp{2}.npy'.format(out_path, name_file, i))#return_dict[i][0] #return_dict[i][0]
