...
def run_one_r_sec(pid, scan_name, step, resolution, duration, bound, asp_cal, start, end, dis_map, slope, inter, cut, tmp_dir, return_dict):
    ...
    for initial_sec in range(start, end):
      ...
      for sec in range(num_co):
        ...
        data_sec, time_sec = dis_correct(np.array(data[sec], dtype='float64'), asp_cal, dis_map, ya_corr, q_corr, cut)
...
#hpc single, identify outliers, 0.5s, secondary co
  if True:
    ...
    for pid in range(p_num-1):
      ...
      p = Process(target=run_one_r_sec, args=(pid, name, step, resolution, duration, bound, asp_cal, start, end, dis_map, ya_corr, q_corr, cut, tmp_dir, return_dict))
      p.start()
      p_list.append(p)
