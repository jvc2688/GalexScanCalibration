...
def no_dis(data, dis_map, asp):
    ...
    ra, dec = gn.gnomrev_simple(data[:,6], data[:,7], asp[ix,1], asp[ix,2], -asp[ix,3],1/36000.,0.)
    return data[:,8:10]#np.array([ra,dec]).T
...
def run_one_r_sec(pid, scan_name, step, resolution, asp_cal, t0, t1, duration, dis_map, outdir, slope, inter, return_dict):
    ...
    for start in range(t0, t1, duration):
      ...
      for initial_sec in range(start, start+duration):
        ...
        for sec in range(num_co):
          ...
          print('correct')
          data_sec, info = dis_correct(np.array(data[sec], dtype='float64'), dis_map, asp_cal, slope, inter)
          #data_sec = no_dis(np.array(data[sec], dtype='float64'), dis_map, asp_cal)
      ...
      name = re.split('_', scan_name)[3]
      bins = np.arange(33)
      plt.hist(info_list[:,2], bins, alpha=0.5)
      plt.xlabel('q')
      plt.title('scan '+name)
      plt.xlim(0,32)
      plt.savefig(outdir+'/q_hist_{0}.pdf'.format(initial_sec))
      plt.clf()

      q_mask = info_list[:,2]>5
      data1 = data1[q_mask]
      info_list = info_list[q_mask]

      print data1.shape
      print data2.shape
      print info_list.shape
      centroid, max_value, flux = get_corr_map(data2, data1, skypos, skyrange, sec, 0.0001, initial_sec,\
                                 '{0}'.format(start), info_list, data3, outdir)
      print centroid
...
#hpc single, identify outliers, 0.5s, secondary co
  if True:
    ...
    suffix = '-correct'
    hdulist = pyfits.open('../AIS_GAL_SCAN/asprta/%s%s-asprta.fits'%(name,suffix))
    co_data = hdulist[1].data
    length = co_data.shape[0]
    ...
    path = os.path.dirname('{0}/test'.format(outdir))
    ...
    pid=0
    run_one_r_sec(pid, name, step, resolution, asp_cal, start, end, duration,  dis_map, outdir, slope, inter, return_dict)
  '''
    p_list = []
    pid = 0
