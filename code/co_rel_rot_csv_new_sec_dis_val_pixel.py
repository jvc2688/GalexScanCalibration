...
def get_corr_map(coo1, coo2, skypos, skyrange, sec, pixsz, idx, suffix, weight=None):
  ...
  #plt.show()
  plt.savefig('/home/dw1519/dw1519/galex/plots/co239_pixel_bp/{0}_log_zoom_c.pdf'.format(idx))
  plt.clf()

  plt.imshow(hp,interpolation='None', cmap=plt.get_cmap('Greys'), aspect='equal', extent=[0.02*3600, -0.02*3600, 0.02*3600, -0.02*3600], origin='upper')  
  plt.colorbar()
  #plt.xlim(-0.02*3600, 0.02*3600)
  #plt.ylim(-0.02*3600, 0.02*3600)
  plt.xlim(-0.003*3600, 0.003*3600)
  ...
  plt.plot(centroid[0]*3600,centroid[1]*3600,'+r', markersize=8)
  #plt.show()
  plt.savefig('/home/dw1519/dw1519/galex/plots/co239_pixel_bp/{0}_zoom_c.pdf'.format(idx))
  plt.clf()
...
def run_one_r_sec(pid, scan_name, step, resolution, asp_cal, start, end, dis_map, return_dict):
    ...
    start = 700
    c_list = []
    for start in range(401, 1200, 100):
      print start
      data1 = []
      data2 = []
      for initial_sec in range(start, start+100):
      ...
      centroid, max_value, flux = get_corr_map(data2, data1, skypos, skyrange, sec, 0.0001, initial_sec, '{0}'.format(start))
      print centroid
      c_list.append(centroid)
    np.save('/home/dw1519/dw1519/galex/plots/co239_pixel/c_list_zoom_c.npy', c_list)
...
if __name__ == '__main__':
  ...
  if True:
    ...
    for i in range(0,18):
      left = 290#1200
      right = 2545#3900
      up = 37904-step*(i+1)#50800-step*(i+1)
      down = 37904-step*i#50800-step*i
      sec = i
      hdu_list = pyfits.open('{0}'.format(name))
      image = hdu_list[0].data[up:down, left:right]
      wcs = WCS(hdu_list[0].header)
      print 'data loaded'
      print image.shape

      row = np.repeat(np.arange(down-up), right-left)+up+1.
      col = np.tile(np.arange(right-left), down-up)+left+1.

      coo = np.array(wcs.all_pix2world(col, row, 1)).T
      weight = image.flatten()
      print coo
      print weight.shape
      ...
      l = np.max(coo[:,0])
      r = np.min(coo[:,0])
      star = catalog_fits.get_catalog_tycho_square(u,d,l,r)
      print star
      print star.shape

      #centroid, max_value, flux = get_corr_map(star, coo, skypos, skyrange, 0, 0.0002, sec, '{0}-{1}_{2}-{3}'.format(up,down,left,right), weight)
      centroid, max_value, flux = get_corr_map(star, coo, skypos, skyrange, 0, 0.0005555555556, sec, '{0}-{1}_{2}-{3}'.format(up,down,left,right), weight)
