...
def get_data(tranges, scan_num):
  look_up = load_obj('lookup%s_new'%scan_num)
  data = []
  trange = [0 , 0]
  ...
  for i in range(200):
    if '%d'%(initial_time+i*5) in look_up:
      initial_time = initial_time+i*5
      print 'initial found:%d'%initial_time
      break


  for i in range(200):
    if '%d'%(final_time-i*5) in look_up:
      final_time = final_time-i*5
  ...
  for i in range(start_index, end_index+1):
    csv_list.append('../data/split%s_new/%d.csv'%(scan_num, i))
...
if __name__ == '__main__':
  ...
  if True:
    ...
    data = get_data(tranges, scan_num)
    print len(data)
    ...
    np.save('../data/%s_new/cata/centroids%d.npy'%(scan_num, initial_sec), centroids)
