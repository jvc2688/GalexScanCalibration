...
def run_one_inter_fast(pid, file_name, imsz, wcs, csv_list, suffix, asp, scst_time, hv, dis_map, skypos, skyrange, return_dict):
  ...
  for csv_file in csv_list:
    ...
    gal = sky_data.transform_to(Galactic)
    data = np.concatenate((np.array([gal.l.deg]).T, np.array([gal.b.deg]).T), axis=1)
    gl_mask = (data[:,1]>skypos[1]-skyrange[1]/2.+0.1) & (data[:,1]<skypos[1]+skyrange[1]/2.+0.1)
    data = data[gl_mask]
    if len(data) == 0:
      continue
...
#process interpolate fast
  if True:
    ...
    for csv_info in csv_info_list:
      ...
      try:
        asp = np.load('../data/photon_list/{0}{1}-new_asp_new.npy'.format(name, suffix))
      except IOError:
        co_data = pyfits.open('../AIS_GAL_SCAN/asprta/{0}{1}-new-asprta.fits'.format(name, suffix))[1].data
        T = co_data['T']
        ra = co_data['ra']
        dec = co_data['dec']
        roll = co_data['roll']
        ...
        np.save('../data/photon_list/{0}{1}-new_asp_new.npy'.format(name, suffix), asp)
      ...
      for chunk in chunks:
        print len(chunk)
        p = Process(target=run_one_inter_fast, args=(pid, name, imsz, wcs, chunk, suffix, asp, scst_time, hv, dis_map, skypos, skyrange, return_dict))
        p.start()
        p_list.append(p)
        pid += 1
    print pid
