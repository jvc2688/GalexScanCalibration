...
if __name__ == '__main__':
    if True:
        ...
        for scan in scans:
            ...
            gl_list.append(gl)
            gb_list.append(gb)
            nuv_ais_list.append(ais['nuv'][idx[mask]])
            nuv_sex_list.append(df['nuv'][mask])

        gl = np.concatenate(gl_list, axis=0)
        gb = np.concatenate(gb_list, axis=0)
        nuv_ais = np.concatenate(nuv_ais_list, axis=0)
        nuv_sex = np.concatenate(nuv_sex_list, axis=0)
        f, (ax1) = plt.subplots(1, sharex=True, figsize=(8,5))
        cax = ax1.scatter(nuv_sex, nuv_ais,
            alpha=0.8, edgecolors='face', s=0.5, cmap=plt.get_cmap('jet'))

        #cbar = f.colorbar(cax, ticks=[-10, 0, 10],orientation='horizontal')
        ax1.plot(np.arange(20), np.arange(20),'-r')
        ax1.set_ylim(8.,22.)
        ax1.set_xlim(8.,22.)
        ax1.set_ylabel('NUV AIS')
        ax1.set_xlabel('NUV SExtractor')

        plt.tight_layout()
        f.subplots_adjust(hspace=0)

        plt.savefig('/scratch/dw1519/galex/fits/scan_map/catalog/'+date+'/all_match_exp_new.png', dpi=190)
        plt.clf()
