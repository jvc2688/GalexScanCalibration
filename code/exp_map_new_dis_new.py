...
def exp_dead_new(file_num, name_file, imsz, wcs, hrflat, asp_solution, dead, return_dict):
	...
	for i in range(asp_solution.shape[0]):
		...
		if i%2000==0:
			with open('../fits/scan_map/%s_gal_sec_exp_tmp%d.npy'%(name_file, file_num),'w') as f:
				f.write('%d'%i)
			print i
...
#process
	if True:
		...
		for name in name_list:
			try:
				asp_solution_tmp = np.load('../data/photon_list/{0}{1}-new_asp_new.npy'.format(name, suffix))
			except IOError:
				...
				np.save('../data/photon_list/{0}{1}-new_asp_new.npy'.format(name, suffix), asp_solution_tmp)
		...
		hdulist.writeto('../fits/scan_map/count_map_{0}{1}_exp.fits'.format(name_file, suffix), clobber=False)
