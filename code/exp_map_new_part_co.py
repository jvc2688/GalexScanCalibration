...
def exp_dead_new(file_num, name_file, npart, imsz, wcs, hrflat, asp_solution, dead, return_dict):
	count = np.zeros(imsz)
	...
	for i in range(asp_solution.shape[0]):
		...
		count[xl:xh, yl:yh] += res[res_xl:res_xh, res_yl:res_yh]*0.01*(1-dead[i])
		...
		if i%2000==0:
			with open('../fits/scan_map/%s_gal_sec_exp_part%d_tmp%d.dat'%(name_file, npart, file_num), 'w') as f:
				f.write('%d'%i)
			print i
	print '%d done'%file_num
	#return_dict[file_num] = count
	np.save('../fits/scan_map/%s_gal_sec_exp_part%d_tmp%d.npy'%(name_file, npart, file_num), count)
...
#process
	if True:
		...
		for name in name_list:
			try:
				asp_solution_tmp = np.load('../data/photon_list/{0}{1}_asp_new_co.npy'.format(name, suffix))
			except IOError:
				...
				dec = co_data['dec']
				roll = co_data['roll']
				t_new = np.arange((T.shape[0]-1)*100)*0.01+T[0]
				...
				np.save('../data/photon_list/{0}{1}_asp_new_co.npy'.format(name, suffix), asp_solution_tmp)
		...
		asp_part = np.array_split(asp_solution, total_part)[npart-1]
		dead_part = np.array_split(dead, total_part)[npart-1]

		asp_solutions = np.array_split(asp_part, num_t)
		deads = np.array_split(dead_part, num_t)
		...
		for file_num in range(0, num_t):
			print file_num
			print asp_solutions[file_num].shape
			print deads[file_num].shape
			p = Process(target=exp_dead_new, args=(file_num, name_file, npart, imsz, wcs, hrflat, asp_solutions[file_num], deads[file_num], return_dict))
			p.start()
			p_list.append(p)
		...
		for i in range(0, num_t):
			count += np.load('../fits/scan_map/%s_gal_sec_exp_part%d_tmp%d.npy'%(name_file, npart, i)) #return_dict[i]
			os.remove('../fits/scan_map/%s_gal_sec_exp_part%d_tmp%d.npy'%(name_file, npart, i))

		tranges.append(trange)
		hdu = pyfits.PrimaryHDU(count)
		hdu = imagetools.fits_header('NUV', skypos, tranges, skyrange, hdu=hdu, wcs=wcs)
		hdulist = pyfits.HDUList([hdu])
		hdulist.writeto('../fits/scan_map/count_map_{0}{1}_exp_part{2}.fits'.format(name_file, suffix, npart), clobber=False)
